...
class ProductDatasourceController(private val productDatasourceService: ProductDatasourceService, private val productDatasourceStateProvider: ProductDatasourceStateProvider) {
    ...
    fun fix() {
        for (productDatasource in productDatasourceService.productDatasourceList()) {
            ...
            if (state.status == ProductDatasourceStatus.RUNNING) {
                productDatasourceStateProvider.saveState(ProductDatasourceState(state.id, state.name, ProductDatasourceStatus.FAILED, state.lastRun))
            }
        }
